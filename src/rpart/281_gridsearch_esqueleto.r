...
for (vcp in seq(0, 0.1, by = 0.01)) { # cp
  for (vmax_depth in c(4, 6, 8, 10, 12, 14)) { # maxdepth
    for (vmin_split in c(10, 20, 50, 100, 200, 400, 600, 800, 1000)) { # minsplit
      for (vmin_bucket in c(5, 10, 15, 20)) { # minbucket
        param_basicos <- list(
          "cp" = vcp,
          "maxdepth" = vmax_depth,
          "minsplit" = vmin_split,
